...
@property (nonatomic, strong) MBProgressHU_DDD *HUD;
...
- (void)initHUBViewWithView:(UIView *)view {
    ...
    if (_HUD == nil) {
        _HUD = [[MBProgressHU_DDD alloc] initWithView:view];
        _HUD.opacity = 0.4;
        ...
        _HUD.mode = MBProgressHU_DDDModeCustomView;
    }
    if ([view isKindOfClass:[UIWindow class]]) {
        [view addSubview:_HUD];
    } else {
