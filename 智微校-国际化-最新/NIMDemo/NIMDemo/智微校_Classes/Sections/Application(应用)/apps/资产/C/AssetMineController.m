...
@interface AssetMineController ()<UIScrollViewDelegate>
...
@implementation AssetMineController
- (NSMutableArray *)dataArray {
    
    if (_dataArray == nil) {
        
        self.dataArray = [[NSMutableArray alloc] init];
        NSArray * array = [NSArray arrayWithObjects:@{@"title":NSLocalizedString(@"APP_assets_myAsset", nil),@"status":@"0"},@{@"title":NSLocalizedString(@"APP_assets_ApplicationList", nil),@"status":@"0"},@{@"title":NSLocalizedString(@"APP_assets_hasReturn", nil),@"status":@"0"}, nil];
...
- (void)setupChildVc {
    for (int i = 0; i <= _indexAll; i ++) {
        AssetMineDetailController * ContentView = [[AssetMineDetailController alloc] init];
