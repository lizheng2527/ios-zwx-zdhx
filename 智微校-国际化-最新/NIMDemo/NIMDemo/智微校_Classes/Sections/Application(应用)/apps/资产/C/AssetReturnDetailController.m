...
@interface AssetReturnDetailController ()<UITableViewDelegate,UITableViewDataSource>


@end

@implementation AssetReturnDetailController
{
...
- (void)viewDidLoad {
    ...
    self.title = NSLocalizedString(@"APP_assets_returnDetail", nil);
    [self requestData];
    [self createBarItem];
...
-(void)requestData
{
    ...
    [helper getReturnDetailJsonWithReturnId:_returnID andStatus:^(BOOL successful, AssetMineReturnModel *returnModel) {
...
-(void)initView
{
    ...
    _returnPersonLabel.text = model.userName;
    _returnHandlePersonLabel.text = model.operatorName;
    _returnOrganizationLabel.text = model.departmentName;
