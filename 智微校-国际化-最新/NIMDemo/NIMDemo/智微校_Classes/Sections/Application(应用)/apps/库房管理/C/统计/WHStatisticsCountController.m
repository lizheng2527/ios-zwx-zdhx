...
@interface WHStatisticsCountController ()<UITableViewDelegate,UITableViewDataSource,WHStatisticsCountCellDelegate>
@property(nonatomic,retain)NSMutableArray *dataArray;
@end

@implementation WHStatisticsCountController
...
-(instancetype)initWithGoodsID:(NSString *)goodID navTitle:(NSString *)navTitle
{
    ...
    if (self) {
        _goodsID = goodID;
        [self setNavTitle:NSLocalizedString(@"APP_wareHouse_whDetail", nil) secondTitle:navTitle];
    }
    return self;
}
...
-(void)requestData
{
    ...
    [helper getGoodsStasticsStockListWithgoodsID:_goodsID andStatus:^(BOOL successful, NSMutableArray *dataSource) {
...
{
    static NSString *iden1 = @"WHStatisticsCountCell";
    WHStatisticsCountCell *cell = [tableView dequeueReusableCellWithIdentifier:iden1];
    if (!cell) {
        cell =[[WHStatisticsCountCell alloc]init];
    }
//    cell.delegate = self;
...
{
    WHStatisticsCountHeaderView *headView = [[WHStatisticsCountHeaderView alloc]initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 44)];
    return headView;
}
