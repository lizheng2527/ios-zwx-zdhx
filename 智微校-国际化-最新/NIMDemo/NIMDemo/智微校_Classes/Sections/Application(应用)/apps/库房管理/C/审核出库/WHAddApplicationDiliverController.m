...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        cell.userPersonLabel.text = outUserName.length?outUserName:NSLocalizedString(@"APP_wareHouse_clickToChoose", nil);
        ...
        [cell.outWarehouseBtn setTitle:outWareHouseName.length?outWareHouseName:NSLocalizedString(@"APP_wareHouse_clickToChoose", nil) forState:UIControlStateNormal];
...
-(void)diliverAction:(id)sender
{
    if ([NSString isBlankString:outWareHouseID]) {
        [self.view makeToast:NSLocalizedString(@"APP_wareHouse_plzChooseWH", nil) duration:1.5 position:CSToastPositionCenter];
        return;
    }
    else if ([NSString isBlankString:outUserID] || [NSString isBlankString:outUserName]) {
        [self.view makeToast:NSLocalizedString(@"APP_wareHouse_plzChooseUser", nil) duration:1.5 position:CSToastPositionCenter];
        return;
    }
    if (_dataArray.count == 0) {
        [self.view makeToast:NSLocalizedString(@"APP_wareHouse_plzAddThings", nil) duration:1.5 position:CSToastPositionCenter];
        return;
    }
    else
    {
...
-(void)applyOutWarehouseBtnDidClick:(WHDiliverMainCell *)cell
{
    ...
    chooseView.typeString = NSLocalizedString(@"APP_wareHouse_chooseType", nil);
...
-(void)applyUserBtnDidClick:(WHDiliverMainCell *)cell
{
        ...
        baseVc.title = NSLocalizedString(@"APP_wareHouse_chooseApplyPerson", nil);
