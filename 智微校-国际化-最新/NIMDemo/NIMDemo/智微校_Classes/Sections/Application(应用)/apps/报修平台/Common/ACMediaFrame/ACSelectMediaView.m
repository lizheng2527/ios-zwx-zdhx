...
-(void)setImageMaxCount:(NSInteger)imageMaxCount
{
    if (!imageMaxCount ) {
        _imageMaxCount = 6;
    }
    else _imageMaxCount = imageMaxCount;
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == _mediaArray.count && _mediaArray.count >= self.imageMaxCount) {
        [UIAlertController showAlertWithTitle:[NSString stringWithFormat:@"最多只能选择%ld张",(long)self.imageMaxCount ] message:nil actionTitles:@[NSLocalizedString(@"APP_General_Confirm", nil)] cancelTitle:nil style:UIAlertControllerStyleAlert completion:nil];
        return;
    }
    ...
    if (indexPath.row == _mediaArray.count) {
        switch (_type) {
            ...
            {
                [UIAlertController showAlertWithTitle:NSLocalizedString(@"APP_Circle_chooseImageSource", nil) message:nil actionTitles:@[NSLocalizedString(@"APP_YUNXIN_fromAlbum", nil), NSLocalizedString(@"APP_YUNXIN_takePic", nil)] cancelTitle:NSLocalizedString(@"APP_General_Cancel", nil) style:UIAlertControllerStyleActionSheet completion:^(NSInteger index) {
                    if (index == 0) {
                        [weakSelf openAlbum];
                    }else {
                        [weakSelf openCamera];
...
/** 相册 */
- (void)openAlbum {
    TZImagePickerController *imagePickerVc = [[TZImagePickerController alloc] initWithMaxImagesCount:self.imageMaxCount - _mediaArray.count delegate:self];
    //是否 在相册中显示拍照按钮
...
- (void)openCamera {
    ...
    if ([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeCamera]){
        ...
        [UIAlertController showAlertWithTitle:@"该设备不支持拍照" message:nil actionTitles:@[NSLocalizedString(@"APP_General_Confirm", nil)] cancelTitle:nil style:UIAlertControllerStyleAlert completion:nil];
    }
}
...
- (void)openVideotape {
    ...
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        [UIAlertController showAlertWithTitle:@"当前设备不支持录像" message:nil actionTitles:@[NSLocalizedString(@"APP_General_Confirm", nil)] cancelTitle:nil style:UIAlertControllerStyleAlert completion:nil];
    }
    [rootVC presentViewController:picker animated:YES completion:nil];
