...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        //内容区的背景
        _contentBgView = [[UIView alloc] initWithFrame:CGRectMake((SCREEN_WIDTH-UNIT_WIDTH*kRow)/2.0, 50, UNIT_WIDTH*kRow, 42+UNIT_WIDTH*kCol+50)];
        ...
        [_done setTitle:NSLocalizedString(@"APP_General_Confirm", nil) forState:UIControlStateNormal];
...
//            [btn setBackgroundColor:[UIColor clearColor]];
            ...
            self.btn.titleEdgeInsets = UIEdgeInsetsMake(0, 0, 0, 0);

            [self.btn setImage:nil forState:UIControlStateNormal];
            [self.btn setTitleColor:[UIColor hexColorWithString:@"2b2b2b"] forState:UIControlStateNormal];
...
+(int)compareDate:(NSString *)date1 date:(NSString *)date2
{
    ...
    NSDate *dateA = [dateFormatter dateFromString:date1];
...
// HH:mm
+(int)compareDate2:(NSString *)date1 date:(NSString *)date2
{
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
