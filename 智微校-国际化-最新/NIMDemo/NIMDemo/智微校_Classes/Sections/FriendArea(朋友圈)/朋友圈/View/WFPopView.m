...
@property(nonatomic, strong)  UIButton *lineLabel;
...
//        //replyButton   likeButton  Frame互换
...
- (UIButton *)replyButton {
    ...
    if (!_likeButton) {
        ...
        [_likeButton addTarget:self action:@selector(operationDidClicked:) forControlEvents:UIControlEventTouchUpInside];
        //replyButton   likeButton  Frame互换
        _likeButton.frame = CGRectMake(0, 0, kXHALbumOperationViewSize.width / 2.0, kXHALbumOperationViewSize.height);
        [_likeButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        _likeButton.titleLabel.font = [UIFont systemFontOfSize:14];
    }
    return _likeButton;
...
- (void)showAtView:(UITableView *)containerView rect:(CGRect)targetRect isFavour:(BOOL)isFavour {
    ...
    [UIView animateWithDuration:0.2 delay:0 options:UIViewAnimationOptionCurveEaseIn animations:^{
        ...
        [_replyButton setTitle:NSLocalizedString(@"APP_Circle_pinglun", nil) forState:UIControlStateNormal];
        [_likeButton setTitle:(isFavour?NSLocalizedString(@"APP_Circle_CancelZan", nil) :NSLocalizedString(@"APP_Circle_zan", nil)) forState:UIControlStateNormal];
    }];
    _lineLabel = [[UILabel alloc]initWithFrame:CGRectMake(_replyButton.frame.origin.x , 8, 1, 18)];
