...
- (void)initLayer {
    ...
    _itemFontSize = 15.0;
    _itemsFont = [UIFont systemFontOfSize:15.0];
    _DirectionDown = YES;
}
...
-(void)didTap : (UIGestureRecognizer *)gesture {
    ...
    if(!isCollapsed) {
        CGFloat shouldExpendHeight = SCREEN_HEIGHT - 84 - 64;
        
        CGFloat height = (CGFloat)_items.count * _itemHeight > shouldExpendHeight ? shouldExpendHeight :  _itemHeight * (double)(_items.count);
...
//    label.text = _items[SelectedIndex];
    label.text = [NSString stringWithFormat:@"教学周: %@",_items[SelectedIndex]];
