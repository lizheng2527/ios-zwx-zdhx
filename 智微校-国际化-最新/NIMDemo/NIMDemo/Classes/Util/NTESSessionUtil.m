...
+ (NSString *)tipOnMessageRevoked:(id)message
{
    ...
    NSString *tip = NSLocalizedString(@"APP_YUNXIN_tip_U", nil);
    if (!isFromMe) {
        switch (session.sessionType) {
            case NIMSessionTypeP2P:
                tip = NSLocalizedString(@"APP_YUNXIN_tip_duifang", nil);
                break;
    ...
    return [NSString stringWithFormat:@"%@%@",tip,NSLocalizedString(@"APP_YUNXIN_tip_bcakMessage", nil)];
}
...
+ (NSString *)onlineState:(NSString *)userId detail:(BOOL)detail
{
    ...
    if ([[NIMSDK sharedSDK].robotManager isValidRobot:userId]) {
        return NSLocalizedString(@"APP_General_onLine", nil);
    }
    ...
    if ([info isKindOfClass:[NIMSubscribeOnlineInfo class]] && info.senderClientTypes.count)
    {
        ...
        switch (event.value) {
            ...
            default:
            {
                ...
                state = [NSString stringWithFormat:@"%@%@",clientName,NSLocalizedString(@"APP_General_onLine", nil)];
                break;
            }
        }
    }
    ...
    {
        state = NSLocalizedString(@"APP_General_offLine", nil);
    }
    return state;
}
...
{
    NSString *clientName = [self resolveOnlineClientName:client];
    NSString *state = [NSString stringWithFormat:@"%@%@",clientName,NSLocalizedString(@"APP_General_onLine", nil)];
    ...
    if (dict) {
        ...
        switch (onlineState) {
            case NTESOnlineStateNormal:
            {
                ...
                {
                    //桌面端不显示网络状态，只显示端
                    return [NSString stringWithFormat:@"%@%@",clientName,NSLocalizedString(@"APP_General_onLine", nil)];
                }
                else
                {
                    ...
                    if (detail)
                    {
                        return [NSString stringWithFormat:@"%@ - %@ %@",clientName,netState,NSLocalizedString(@"APP_General_onLine", nil)];
                    }
                    else
                    {
                        return [NSString stringWithFormat:@"%@ %@",netState,NSLocalizedString(@"APP_General_onLine", nil)];
                    }
                }
            }
            case NTESOnlineStateBusy:
                return NSLocalizedString(@"APP_YUNXIN_tip_Manglu", nil);
            case NTESOnlineStateLeave:
                return NSLocalizedString(@"APP_YUNXIN_tip_goAway", nil);
            default:
                break;
        }
    }
...
{
    NSString *message = [NSString stringWithFormat:@"%@ %@",NSLocalizedString(@"APP_YUNXIN_tip_autoLoginFailed", nil) ,error];
    ...
    if ([domain isEqualToString:NIMLocalErrorDomain])
    {
        ...
        {
            message = NSLocalizedString(@"APP_YUNXIN_tip_autoLoginFailedWIthNetCheck", nil);
        }
    }
    else if([domain isEqualToString:NIMRemoteErrorDomain])
    {
        ...
        {
            message = NSLocalizedString(@"APP_YUNXIN_tip_pwError", nil);
        }
        else if(code == NIMRemoteErrorCodeExist)
        {
            message = NSLocalizedString(@"APP_YUNXIN_tip_otherLogining", nil);
        }
    }
    return message;
}
