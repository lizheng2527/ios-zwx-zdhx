...
- (void)setUpNav{
    self.navigationItem.title = NSLocalizedString(@"APP_YUNXIN_PersonalCard_card", nil);
    if ([self.userId isEqualToString:[[NIMSDK sharedSDK].loginManager currentAccount]]) {
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"APP_YUNXIN_PersonalCard_edit", nil) style:UIBarButtonItemStyleDone target:self action:@selector(onActionEditMyInfo:)];
...
- (void)buildData{
    ...
    NSArray *data = @[
                        ...
                        @{
                            ...
                            RowContent :@[
                                           @{
                                                Title        : NSLocalizedString(@"APP_YUNXIN_PersonalCard_nickName", nil),
                         ...
                         @{
                            ...
                            RowContent :@[
                                    @{
                                        Title        : NSLocalizedString(@"APP_YUNXIN_PersonalCard_notification", nil),
                          ...
                          @{
                            ...
                            RowContent :@[
                                           @{
                                                Title        : NSLocalizedString(@"APP_YUNXIN_Contact_blackList", nil),
                                                CellClass    : @"NTESSettingSwitcherCell",
                          ...
                          @{
                            ...
                            RowContent :@[
                                    @{
                                        Title        : NSLocalizedString(@"APP_YUNXIN_PersonalCard_talk", nil),
                                    ...
                                    @{
                                        Title        : NSLocalizedString(@"APP_YUNXIN_PersonalCard_delFriend", nil),
                                        CellClass    : @"NTESColorButtonCell",
                                    ...
                                    @{
                                        Title        : NSLocalizedString(@"APP_YUNXIN_PersonalCard_addFriend", nil),
                                        CellClass    : @"NTESColorButtonCell",
...
- (void)onActionBlackListValueChange:(id)sender{
    ...
    if (switcher.on) {
        [[NIMSDK sharedSDK].userManager addToBlackList:self.userId completion:^(NSError *error) {
            ...
            if (!error) {
                [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_PersonalCard_takeToBlackListSuccess", nil) duration:2.0f position:CSToastPositionCenter];
            }else{
                [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_PersonalCard_takeToBlackListFailed", nil) duration:2.0f position:CSToastPositionCenter];
                [wself refresh];
            }
        }];
    }else{
        [[NIMSDK sharedSDK].userManager removeFromBlackBlackList:self.userId completion:^(NSError *error) {
            [SVProgressHUD dismiss];
            if (!error) {
                [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_PersonalCard_removeBlackListSuccess", nil) duration:2.0f position:CSToastPositionCenter];
            }else{
                [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_PersonalCard_removeBlackListFailed", nil) duration:2.0f position:CSToastPositionCenter];
                [wself refresh];
            }
        }];
    }
...
- (void)onActionNeedNotifyValueChange:(id)sender{
    ...
    [[NIMSDK sharedSDK].userManager updateNotifyState:switcher.on forUser:self.userId completion:^(NSError *error) {            [SVProgressHUD dismiss];
        if (error) {
            [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_Contact_blackList_operationFailed",nil) duration:2.0f position:CSToastPositionCenter];
            [wself refresh];
        }
    }];
}
...
- (void)addFriend{
    ...
    if ([[NTESBundleSetting sharedConfig] needVerifyForFriend]) {
        ...
        request.message = NSLocalizedString(@"APP_YUNXIN_PersonalCard_pleasPass", nil);
    }
    NSString *successText = request.operation == NIMUserOperationAdd ? NSLocalizedString(@"APP_YUNXIN_PersonalCard_addSuccess", nil) : NSLocalizedString(@"APP_YUNXIN_PersonalCard_requestSuccess", nil);
    NSString *failedText =  request.operation == NIMUserOperationAdd ? NSLocalizedString(@"APP_YUNXIN_PersonalCard_Addfailed", nil) : NSLocalizedString(@"APP_YUNXIN_PersonalCard_requestFailed", nil);
    ...
    [alert showAlertWithCompletionHandler:^(NSInteger index) {
        if (index == 1) {
            ...
            [[NIMSDK sharedSDK].userManager deleteFriend:wself.userId completion:^(NSError *error) {
                [SVProgressHUD dismiss];
                if (!error) {
                    [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_Contact_delSuccess", nil) duration:2.0f position:CSToastPositionCenter];
                    [wself refresh];
                }else{
                    [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_Contact_delFailed", nil) duration:2.0f position:CSToastPositionCenter];
                }
            }];
        }
    }];
...
- (void)onLongPressNavbar:(UIGestureRecognizer *)gesture{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"APP_YUNXIN_PersonalInfo", nil) message:title delegate:nil cancelButtonTitle:NSLocalizedString(@"APP_General_Confirm", nil) otherButtonTitles:nil, nil];
        [alert show];
    }
}
