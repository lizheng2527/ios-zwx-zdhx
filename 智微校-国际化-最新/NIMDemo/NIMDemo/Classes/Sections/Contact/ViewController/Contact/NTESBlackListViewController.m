...
- (void)viewDidLoad {
    ...
    [self.header refreshWithType:ListHeaderTypeCommonText value:NSLocalizedString(@"APP_YUNXIN_Contact_blackList_notReceive", nil)];
    [self.view addSubview:self.header];
}


- (void)setUpNavItem{
    self.navigationItem.title = NSLocalizedString(@"APP_YUNXIN_Contact_blackList", nil);
...
- (void)didFinishedSelect:(NSArray *)selectedContacts{
    if (selectedContacts.count) {
        ...
        [[NIMSDK sharedSDK].userManager addToBlackList:selectedContacts.firstObject completion:^(NSError *error) {
            if (!error) {
                [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_Contact_blackList_operationSuccess", nil) duration:2.0 position:CSToastPositionCenter];
                wself.data = wself.myBlackListUser;
                [wself.tableView reloadData];
            }else{
                [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_Contact_blackList_operationFailed", nil) duration:2.0 position:CSToastPositionCenter];
            }
        }];
    }
}
