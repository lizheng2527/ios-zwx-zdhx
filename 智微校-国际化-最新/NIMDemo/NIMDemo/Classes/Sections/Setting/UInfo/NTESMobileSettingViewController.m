...
- (void)setUpNav{
    self.navigationItem.title = NSLocalizedString(@"APP_YUNXIN_phoneNumber", nil);
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"APP_YUNXIN_phoneFinish", nil) style:UIBarButtonItemStyleDone target:self action:@selector(onDone:)];
...
- (void)onDone:(id)sender{
    ...
    if (self.mobile.length > self.inputLimit) {
        [self.view makeToast:NSLocalizedString(@"APP_YUNXIN_phoneNumberTooLong", nil) duration:2.0 position:CSToastPositionCenter];
        return;
    }
    [SVProgressHUD show];
    __weak typeof(self) wself = self;
    [[NIMSDK sharedSDK].userManager updateMyUserInfo:@{@(NIMUserInfoUpdateTagMobile) : self.mobile} completion:^(NSError *error) {
        ...
        if (!error) {
            ...
            [nav.view makeToast:NSLocalizedString(@"APP_YUNXIN_phoneNumberSetSuccess", nil) duration:2.0 position:CSToastPositionCenter];
        }else{
            [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_phoneNumberSetFailed", nil) duration:2.0 position:CSToastPositionCenter];
        }
    }];
}
