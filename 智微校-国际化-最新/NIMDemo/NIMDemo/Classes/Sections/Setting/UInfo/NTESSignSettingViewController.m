...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self setUpNav];
    self.navigationItem.title = NSLocalizedString(@"APP_YUNXIN_SignSet", nil);
...
- (void)onDone:(id)sender{
    ...
    if (self.sign.length > self.inputLimit) {
        [self.view makeToast:NSLocalizedString(@"APP_YUNXIN_SignTooLong", nil) duration:2.0 position:CSToastPositionCenter];
        return;
    }
    [SVProgressHUD show];
    __weak typeof(self) wself = self;
    [[NIMSDK sharedSDK].userManager updateMyUserInfo:@{@(NIMUserInfoUpdateTagSign) : self.sign} completion:^(NSError *error) {
        ...
        if (!error) {
            ...
            [vc.view makeToast:NSLocalizedString(@"APP_YUNXIN_SignSetSuccess", nil)
                         duration:2
                         position:CSToastPositionCenter];
        }else{
            [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_SignSetFailed", nil)
                         duration:2
                         position:CSToastPositionCenter];
        }
    }];
