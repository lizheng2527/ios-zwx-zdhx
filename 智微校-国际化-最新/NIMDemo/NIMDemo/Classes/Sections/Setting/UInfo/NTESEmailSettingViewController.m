...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self setUpNav];
    self.navigationItem.title = NSLocalizedString(@"APP_YUNXIN_emailSet", nil);
...
- (void)setUpNav{
    self.navigationItem.title = NSLocalizedString(@"APP_YUNXIN_email", nil);
...
- (void)onDone:(id)sender{
    ...
    if (self.email.length > self.inputLimit) {
        [self.view makeToast:NSLocalizedString(@"APP_YUNXIN_emailTooLong", nil) duration:2.0 position:CSToastPositionCenter];
        return;
    }
    [SVProgressHUD show];
    __weak typeof(self) wself = self;
    [[NIMSDK sharedSDK].userManager updateMyUserInfo:@{@(NIMUserInfoUpdateTagEmail) : self.email} completion:^(NSError *error) {
        ...
        if (!error) {
            ...
            [vc.view makeToast:NSLocalizedString(@"APP_YUNXIN_emailSetSuccess", nil) duration:2.0 position:CSToastPositionCenter];
        }else{
            [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_emailSetFailed", nil)
                         duration:2
