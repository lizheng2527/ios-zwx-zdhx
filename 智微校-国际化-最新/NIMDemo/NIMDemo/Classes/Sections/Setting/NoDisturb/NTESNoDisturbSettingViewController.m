...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = NSLocalizedString(@"APP_YUNXIN_disturb_setting", nil);
    [self buildData];
...
- (void)buildData{
    ...
    NSArray *data = @[
                      @{
                          ...
                          RowContent :@[
                                  @{
                                      Title        : NSLocalizedString(@"APP_YUNXIN_disturbb", nil),
                                  ...
                                  @{
                                      Title       : NSLocalizedString(@"APP_YUNXIN_disturb_from", nil),
                                  ...
                                  @{
                                      Title      :NSLocalizedString(@"APP_YUNXIN_disturb_to", nil),
                          ...
                          FooterTitle:NSLocalizedString(@"APP_YUNXIN_disturb_notNotifacation", nil)
                          },
                      ];
...
- (void)onActionSetNoDisturbingStart:(id)sender{
    ...
    [pickerView showInView:self.view.window onCompletion:^(NSInteger hour, NSInteger minute) {
        ...
        if (hour == setting.noDisturbingEndH && minute == setting.noDisturbingEndM) {
            [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_disturb_needSame", nil) duration:2.0 position:CSToastPositionCenter];
            [wself refreshData];
        }else{
...
- (void)updateAPNSSetting:(NIMPushNotificationSetting *)setting{
    ...
    [[NIMSDK sharedSDK].apnsManager updateApnsSetting:setting completion:^(NSError *error) {
        wself.isUpdate = YES;
        if (error) {
            [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_disturb_failed", nil) duration:2.0 position:CSToastPositionCenter];
        }
        [wself refreshData];
    }];
}
