...
- (void)refreshWithClients:(NSArray *)clients{
    ...
    if (clients.count) {
        ...
        text = name.length? [NSString stringWithFormat:@"%@%@%@",NSLocalizedString(@"APP_YUNXIN_nowUsingKnow", nil),name,NSLocalizedString(@"APP_YUNXIN_nowUsingBan", nil)] : NSLocalizedString(@"APP_YUNXIN_nowUsingUnKnow", nil);
    }
