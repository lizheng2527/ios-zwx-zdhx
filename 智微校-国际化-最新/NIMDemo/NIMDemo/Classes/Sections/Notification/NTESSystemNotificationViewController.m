...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = NSLocalizedString(@"APP_YUNXIN_Contact_ajaxMessage", nil);
    ...
    if (notifications.count >= MaxNotificationCount) {
        ...
        [button setTitle:NSLocalizedString(@"APP_YUNXIN_Contact_loadMore", nil) forState:UIControlStateNormal];
        self.tableView.tableFooterView = button;
    }else{
        self.tableView.tableFooterView = [[UIView alloc] init];
    }
    
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"APP_YUNXIN_Contact_clear", nil)
    ...
    switch (notification.type) {
        case NIMSystemNotificationTypeTeamApply:{
            [[NIMSDK sharedSDK].teamManager passApplyToTeam:notification.targetID userId:notification.sourceID completion:^(NSError *error, NIMTeamApplyStatus applyStatus) {
                if (!error) {
                    [wself.navigationController.view makeToast:NSLocalizedString(@"APP_YUNXIN_Contact_agreeSuccess", nil)
                                                      duration:2
                    ...
                    if(error.code == NIMRemoteErrorCodeTimeoutError) {
                        [wself.navigationController.view makeToast:NSLocalizedString(@"APP_YUNXIN_Contact_serverError", nil)
                                                          duration:2
                                                          position:CSToastPositionCenter];
                    } else {
        ...
        case NIMSystemNotificationTypeTeamInvite:{
            [[NIMSDK sharedSDK].teamManager acceptInviteWithTeam:notification.targetID invitorId:notification.sourceID completion:^(NSError *error) {
                if (!error) {
                    [wself.navigationController.view makeToast:NSLocalizedString(@"APP_YUNXIN_Contact_acceptSuccess", nil)
                                                      duration:2
                                                      position:CSToastPositionCenter];
                    notification.handleStatus = NotificationHandleTypeOk;
                    [wself.tableView reloadData];
                    ...
                    if(error.code == NIMRemoteErrorCodeTimeoutError) {
                        ...
                        [wself.navigationController.view makeToast:NSLocalizedString(@"APP_YUNXIN_Contact_groupNotExsit", nil)
                                                          duration:2
                                                          position:CSToastPositionCenter];
                    }
                    else {
        ...
        case NIMSystemNotificationTypeFriendAdd:
        {
            ...
            [[[NIMSDK sharedSDK] userManager] requestFriend:request
                                                 completion:^(NSError *error) {
                                                     if (!error) {
                                                         [wself.navigationController.view makeToast:NSLocalizedString(@"APP_YUNXIN_Contact_ajaxSuccess", nil)
                                                                                           duration:2
                                                                                           position:CSToastPositionCenter];
                                                         notification.handleStatus = NotificationHandleTypeOk;
                                                     }
                                                     ...
                                                     {
                                                         [wself.navigationController.view makeToast:NSLocalizedString(@"APP_YUNXIN_Contact_ajaxfailed", nil)
                                                                                           duration:2
                                                                                           position:CSToastPositionCenter];
                                                     }
                                                     [wself.tableView reloadData];
    ...
    switch (notification.type) {
        case NIMSystemNotificationTypeTeamApply:{
            [[NIMSDK sharedSDK].teamManager rejectApplyToTeam:notification.targetID userId:notification.sourceID rejectReason:@"" completion:^(NSError *error) {
                if (!error) {
                    [wself.navigationController.view makeToast:NSLocalizedString(@"APP_YUNXIN_Contact_refuseSuccess", nil)
                                                      duration:2
                                                      position:CSToastPositionCenter];
        ...
        case NIMSystemNotificationTypeFriendAdd:
        {
            ...
            [[[NIMSDK sharedSDK] userManager] requestFriend:request
                                                 completion:^(NSError *error) {
                                                     ...
                                                     {
                                                         [wself.navigationController.view makeToast:NSLocalizedString(@"APP_YUNXIN_Contact_refuseFailed", nil)
                                                                                           duration:2
                                                                                           position:CSToastPositionCenter];
                                                     }
                                                     [wself.tableView reloadData];
