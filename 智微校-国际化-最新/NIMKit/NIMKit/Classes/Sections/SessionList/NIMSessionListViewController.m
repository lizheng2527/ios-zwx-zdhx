...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    _headerView.nameLabel.text = NSLocalizedString(@"APP_YUNXIN_messageNotifacation", nil);
    _headerView.messageLabel.text = NSLocalizedString(@"App_YUNXIN_noPushData", nil);
...
- (NSString *)messageContent:(NIMMessage*)lastMessage{
    ...
    switch (lastMessage.messageType) {
        ...
        case NIMMessageTypeAudio:
            text = NSLocalizedString(@"APP_YUNXIN_AUDIO", nil);
            break;
        case NIMMessageTypeImage:
            text = NSLocalizedString(@"APP_YUNXIN_PIC", nil);
            break;
        case NIMMessageTypeVideo:
            text = NSLocalizedString(@"APP_YUNXIN_VIDEO", nil);
            break;
        case NIMMessageTypeLocation:
            text = NSLocalizedString(@"APP_YUNXIN_LOCATION", nil);
            break;
        case NIMMessageTypeNotification:{
            return [self notificationMessageContent:lastMessage];
        }
        case NIMMessageTypeFile:
            text = NSLocalizedString(@"APP_YUNXIN_FILE", nil);
            break;
...
- (NSString *)notificationMessageContent:(NIMMessage *)lastMessage{
    ...
    if (object.notificationType == NIMNotificationTypeNetCall) {
        ...
        if (content.callType == NIMNetCallTypeAudio) {
            return NSLocalizedString(@"APP_YUNXIN_netCall", nil);
        }
        return NSLocalizedString(@"APP_YUNXIN_VideoCall", nil);
    }
    if (object.notificationType == NIMNotificationTypeTeam) {
        NIMTeam *team = [[NIMSDK sharedSDK].teamManager teamById:lastMessage.session.sessionId];
        if (team.type == NIMTeamTypeNormal) {
            return NSLocalizedString(@"APP_YUNXIN_taolunzuUpdate", nil);
        }else{
            return NSLocalizedString(@"APP_YUNXIN_groupUpdate", nil);
        }
    }
    return NSLocalizedString(@"APP_YUNXIN_unknownMessage", nil);
}
...
- (NSString *)robotMessageContent:(NIMMessage *)lastMessage{
    ...
    if (object.isFromRobot)
    {
        return NSLocalizedString(@"APP_YUNXIN_robotMessage", nil);
    }
    else
    {
...
-(void)changeHeaderViewStateNotification:(NSNotification *)notification{
    _headerView.nameLabel.text = NSLocalizedString(@"APP_YUNXIN_messageNotifacation", nil);
    _headerView.messageLabel.text = NSLocalizedString(@"App_YUNXIN_receiveNewMessage", nil);
