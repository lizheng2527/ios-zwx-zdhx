...
- (void)viewDidLoad {
    ...
    if(_canCreateAnnouncement) {
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"APP_YUNXIN_CreateNew", nil) style:UIBarButtonItemStylePlain target:self action:@selector(onCreateAnnouncement:)];
    }
    ...
    self.navigationItem.title = NSLocalizedString(@"APP_YUNXIN_GroupNote", nil);
