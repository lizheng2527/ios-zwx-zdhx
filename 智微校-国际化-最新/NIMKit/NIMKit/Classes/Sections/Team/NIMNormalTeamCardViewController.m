    ...
    [self requestData:^(NSError *error, NSArray *data) {
        if (!error) {
           [wself refreshWithMembers:data];
            ...
            [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_getDisGroupMemberFailed", nil)];
        }
...
- (void)requestData:(void(^)(NSError *error,NSArray *data)) handler{
    ...
    [[NIMSDK sharedSDK].teamManager fetchTeamMembers:self.team.teamId completion:^(NSError *error, NSArray *members) {
        ...
        if (!error) {
            ...
            [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_UoutDisGroup", nil)];
        }else{
            [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_inviteJoinFriendFailed", nil)];
        }
        handler(error,array);
    }];
}
...
- (NSArray*)buildBodyData{
    ...
    itemName.title            = NSLocalizedString(@"APP_YUNXIN_disGroupName", nil);
    ...
    itemQuit.title            = NSLocalizedString(@"APP_YUNXIN_outDisGroup", nil);
...
- (void)updateTeamInfoName{
    _updateTeamNameAlertView = [[UIAlertView alloc] initWithTitle:@"" message:NSLocalizedString(@"APP_YUNXIN_changeGroupName", nil) delegate:self cancelButtonTitle:NSLocalizedString(@"APP_General_Cancel", nil) otherButtonTitles:NSLocalizedString(@"APP_General_Confirm", nil), nil];
    _updateTeamNameAlertView.alertViewStyle = UIAlertViewStylePlainTextInput;
    [_updateTeamNameAlertView show];
}

- (void)quitTeam{
    _quitTeamAlertView = [[UIAlertView alloc] initWithTitle:@"" message:NSLocalizedString(@"APP_YUNXIN_sureOutdisGroup", nil) delegate:self cancelButtonTitle:NSLocalizedString(@"APP_General_Cancel", nil) otherButtonTitles:NSLocalizedString(@"APP_General_Confirm", nil), nil];
        ...
        switch (self.currentOpera) {
            case CardHeaderOpeatorAdd:{
                ...
                [[NIMSDK sharedSDK].teamManager addUsers:selectedContacts
                                              ...
                                              postscript:NSLocalizedString(@"APP_YUNXIN_invitejoindisGroup", nil)
                    ...
                    if (!error) {
                        if (self.team.type == NIMTeamTypeNormal) {
                            ...
                            [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_invitejoindisGroupSuccess", nil) duration:2.0 position:CSToastPositionCenter];
                        }
                        [wself refreshTableHeader:self.view.nim_width];
                    }else{
                        [wself.view makeToast:NSLocalizedString(@"APP_YUNXIN_groupInviteFailed", nil)];
                    }
