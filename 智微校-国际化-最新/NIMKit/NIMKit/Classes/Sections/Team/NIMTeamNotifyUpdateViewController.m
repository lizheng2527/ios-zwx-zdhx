...
{
    [super viewDidLoad];
    self.navigationItem.title = NSLocalizedString(@"APP_YUNXIN_messageNotifacation", nil);
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"APP_YUNXIN_emailFinish", nil) style:UIBarButtonItemStyleDone target:self action:@selector(onDone:)];
    ...
    if ([[NIMSDK sharedSDK].teamManager notifyStateForNewMsg:self.team.teamId] != self.selectState)
    {
        ...
        [[NIMSDK sharedSDK].teamManager updateNotifyState:self.selectState inTeam:self.team.teamId completion:^(NSError * _Nullable error)
        {
            ...
            if (!error)
            {
                ...
                [weakSelf.navigationController.view makeToast:NSLocalizedString(@"APP_YUNXIN_pw_changeSuccess", nil) duration:2.0 position:CSToastPositionCenter];
            }
            else
            {
                [weakSelf.view makeToast:NSLocalizedString(@"APP_YUNXIN_pw_changeFailed", nil) duration:2.0 position:CSToastPositionCenter];
            }
...
- (NSArray*)buildBodyData
{
    ...
    notifyAll.title               = NSLocalizedString(@"APP_YUNXIN_tip_notiALL", nil);
    ...
    notifyManager.title            = NSLocalizedString(@"APP_YUNXIN_tip_notiAdmin", nil);
    ...
    notifyNone.title               = NSLocalizedString(@"APP_YUNXIN_tip_notiNothing", nil);
